...
`);
    ...
    let emoji = await ConfigMessage.awaitReactions(
      ...
      client.sendTime(
        message.channel, "❌ | **при настройке не втыкай!**"
      );
    ...
    if (em._emoji.name === "1️⃣") {
      await client.sendTime(message.channel, "префикс?");
      let prefix = await message.channel.awaitMessages(
        (msg) => msg.author.id === message.author.id,
        { max: 1, time: 30000, errors: ["time"] }
      );
      if (!prefix.first())
        return client.sendTime(message.channel, "много буков.");
      prefix = prefix.first();
      ...
      client.sendTime(
        message.channel, `prefix as \`${prefix}\``
      );
    } else {
      await client.sendTime(
        message.channel, "роль танспола."
      );
      ...
      if (!role.first())
        return client.sendTime(message.channel, "много юукв.");
      role = role.first();
      ...
      client.sendTime(
        message.channel, "DJ role as <@&" + role.id + ">"
      );
    }
  },
